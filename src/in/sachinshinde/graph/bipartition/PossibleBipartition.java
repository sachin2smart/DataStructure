...
public class PossibleBipartition {
  ...
  private static boolean isBipartite(int n, int[][] dislikes) {
    ...
    for (int currNode = 1; currNode <= n; ++currNode)
      if (color[currNode] == null && !isColoringValid(graph, color, currNode, 1)) return false;

    return true;
  }

  private static boolean isColoringValid(
      List<Integer>[] graph, Integer[] color, int currNode, int currColor) {
  ...
  public static void main(String[] args) {
    ...
    if (isBipartite(4, dislikes)) System.out.println("Bipartite");
    else System.out.println("Not Bipartite"); // result

    dislikes = new int[][] {{1, 2}, {1, 3}, {2, 4}};
    if (isBipartite(4, dislikes)) System.out.println("Bipartite"); // result
