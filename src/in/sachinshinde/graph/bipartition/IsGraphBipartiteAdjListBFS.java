...
public class IsGraphBipartiteAdjListBFS {
  ...
  private static boolean isAdjColoringValid(
      ArrayList<ArrayList<Integer>> adjList, int node, int[] color) {
    ...
    while (!coloredNodesQueue.isEmpty()) {
      ...
      for (Integer adjNode : adjList.get(currNode)) {
        ...
        if (color[adjNode] == -1) {
          color[adjNode] =
              1 - color[currNode]; // color with opposite color of currNode, if currNode is colored
          // with 0 then color adjNode with 1, else if currNode is colored
          // with 1 then color adjNode wth 0
  ...
  public static void main(String[] args) {
    ...
    if (isBipartite(adj, n)) System.out.println("Bipartite");
    else System.out.println("Not Bipartite"); // result
    ...
    if (isBipartite(adj, n)) System.out.println("Bipartite"); // result
