...
public class IsGraphBipartiteAdjMatrixDFS {
  ...
  public static boolean isBipartite(int[][] graph) {
    ...
    for (int node = 0; node < n; node++)
      if (colors[node] == 0 && !validColor(graph, colors, 1, node)) return false;

    return true;
  }

  private static boolean validColor(int[][] graph, int[] colors, int bgColor, int node) {
    if (colors[node] != 0) return colors[node] == bgColor;

    colors[node] = bgColor;

    for (int adjNode : graph[node]) if (!validColor(graph, colors, -bgColor, adjNode)) return false;
  ...
  public static void main(String[] args) {
    ...
    if (isBipartite(graph)) System.out.println("Bipartite");
    else System.out.println("Not Bipartite"); // result

    graph = new int[][] {{1, 3}, {0, 2}, {1, 3}, {0, 2}};
    if (isBipartite(graph)) System.out.println("Bipartite"); // result
