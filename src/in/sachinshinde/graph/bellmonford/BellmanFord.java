...
public class BellmanFord {
  ...
  public static void bellmanFord(ArrayList<Node> edges, int N, int src) {
    ...
    for (int i = 1; i < N - 1; i++) {
      for (Node node : edges) {
        if (dist[node.getU()] + node.getWeight() < dist[node.getV()]) {
          dist[node.getV()] =
              dist[node.getU()] + node.getWeight(); // Always store distance for v'th node
        }
      }
    }
    ...
    // If no negative cycle then dist[] array will represent the distance of each node from given
    // src node
    if (flag == false) {
      for (int i = 0; i < N; i++) {
