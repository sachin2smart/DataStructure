...
public class IslandPerimeter {
  ...
  private int getIslandPerimeter(int[][] grid) {
    ...
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[0].length; j++) {
        if (grid[i][j] == 1) {
          size += 4;
          if (i > 0 && grid[i - 1][j] == 1) size -= 2;
          if (j > 0 && grid[i][j - 1] == 1) size -= 2;
        }
      }
    }
  ...
  private int getPerimeter(int[][] grid, int i, int j) {
    // if it crosses the border - means we have to stop here and return 1 indicating the edge at the
    // border
    ...
    */
    ...
    // if the island is already visited, means associated edge is already being counted for
    // parameter, so exclude it
    if (grid[i][j] == -1) {
      return 0;
    }

    // Mark that the island is visited, don't mark it with 0 otherwise it will difficult to
    // determine which is border and which represent visited
    grid[i][j] = -1;
