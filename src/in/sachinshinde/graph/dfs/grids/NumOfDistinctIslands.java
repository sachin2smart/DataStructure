...
public class NumOfDistinctIslands {
  ...
  public int numDistinctIslands(int[][] grid) {
    ...
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[0].length; j++) {
        if (grid[i][j] == 1) {
          Set<String> currSet =
              new HashSet<>(); //	Second Set - to get islands based on current position
          dfs(grid, i, j, i, j, currSet);
          masterSet.add(
              currSet
                  .toString()); // Since set contains unique elements, repeated will be mapped into
          // existing
        }
      }
    }
  ...
  public void dfs(int[][] grid, int i, int j, int baseX, int baseY, Set<String> currSet) {
    ...
    currSet.add(
        (i - baseX)
            + "_"
            + (j - baseY)); // 	Adding a delimiter will avoid any math operation for indices
