...
public class FindAllPossibleRecipesFromGivenSupplies {
  ...
  public List<String> findAllRecipes2(
      String[] recipes, List<List<String>> ingredients, String[] supplies) {
    Set<String> ingredientSet = new HashSet<String>();
    for (String s : supplies) ingredientSet.add(s);

    Map<String, List<String>> recipeGraph = new HashMap<String, List<String>>();
    for (int i = 0; i < recipes.length; i++) recipeGraph.put(recipes[i], ingredients.get(i));
  ...
  private boolean isPossibleToMakeRecipe(
      Set<String> ingredientSet,
      Set<String> ingredientTaken,
      Map<String, List<String>> recipeGraph,
      String currIngredient) {
    if (ingredientSet.contains(currIngredient)) return true;

    if (ingredientTaken.contains(currIngredient)) return false;

    ingredientTaken.add(currIngredient);

    if (recipeGraph.get(currIngredient) == null) return false;
  ...
  public List<String> findAllRecipes3(
      String[] recipes, List<List<String>> ingredients, String[] supplies) {
