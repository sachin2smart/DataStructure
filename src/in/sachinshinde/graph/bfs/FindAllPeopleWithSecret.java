...
/*
        ...
        More formally, for every meeting,
            if a person xi has the secret at timei, then
...
public class FindAllPeopleWithSecret {
    ...
    //	Video Solution: https://www.youtube.com/watch?v=TdkWz96iGJk
    public List<Integer> findAllPeople(int n, int[][] meetings, int firstPerson) {
        ...
        int[] secretShared = new int[n];
        Arrays.fill(secretShared, Integer.MAX_VALUE);
        
        secretShared[0] = 0;
        secretShared[firstPerson] = 0;
        
        Queue<int[]> meetingQueues = new ArrayDeque<>();
        meetingQueues.offer(new int[]{0, 0});
        meetingQueues.offer(new int[]{firstPerson, 0});

        while (!meetingQueues.isEmpty()) {
            int[] personTime = meetingQueues.poll();
            int person = personTime[0];
            int currPersonTime = personTime[1];

            for (int[] nextPersonTimes : graph.getOrDefault(person, new ArrayList<>())) {
                int nextPersonTime = nextPersonTimes[0];
                int nextPerson = nextPersonTimes[1];
                
                if (nextPersonTime >= currPersonTime && secretShared[nextPerson] > nextPersonTime) {
                    secretShared[nextPerson] = nextPersonTime;
                    meetingQueues.offer(new int[]{nextPerson, nextPersonTime});
                }
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            if (secretShared[i] != Integer.MAX_VALUE) {
        	    res.add(i);
            }
        }
    ...
    public static void main(String[] args) {
        FindAllPeopleWithSecret people = new FindAllPeopleWithSecret();
        int[][] meetings = new int[][]{
                {1,2,5},
                {2,3,8},
                {1,5,10}
        };
        List<Integer> res = people.findAllPeople(6, meetings, 1);
        System.out.println(res);    // [0, 1, 2, 3, 5]
        ...
        res = people.findAllPeople(4, meetings, 3);
        System.out.println(res);    // [0,1,3]
        ...
        res = people.findAllPeople(5, meetings, 1);
        System.out.println(res);    // [0,1,2,3,4]
    }
}