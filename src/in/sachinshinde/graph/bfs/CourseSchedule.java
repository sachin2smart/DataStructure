...
public class CourseSchedule {
  ...
  public boolean canFinish(int numCourses, int[][] prerequisites) {
    ...
    for (int i = 0; i < numCourses; i++) if (inDegree[i] == 0) possibleCourses.add(i);

    // insert the nodes into list with in-degree = 1
    for (int i = 0; i < possibleCourses.size(); i++)
      for (int j : adj[possibleCourses.get(i)]) if (--inDegree[j] == 0) possibleCourses.add(j);
  ...
  public boolean canFinish2(int numCourses, int[][] prerequisites) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int[] pair : prerequisites)
        if (pair[0] == course) if (--indegree[pair[1]] == 0) queue.add(pair[1]);
    }
