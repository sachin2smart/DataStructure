...
//------------------------------------------------------------------------------------------------------------------
    ...
    public int findCheapestPrice_BellmonFord(int n, int[][] flights, int src, int dst, int k) {
        ...
        for(int i = 0; i <= k; i++) {	// check for "k+1" flights, k is not used in looping statements
		...
		if(hm.containsKey(src)) {
			for (int[] flight : hm.get(src)) {
				if (flight[1] + currCost < totalCost) {    //  flight[1] is the cost to reach destination flight[0]
					dfs(hm, flight[0], dst, k - 1, flight[1] + currCost);    // flight[0] is the next stop/layover (dst) from flight[1] (src)
				}
			}
		}
	}
...
//------------------------------------------------------------------------------------------------------------------
	...
	public int findCheapestPrice_Dijkstra(int n, int[][] flights, int src, int dst, int k) {
		...
		while(!pq.isEmpty()) {
			...
			if(numStopToPass > 0) {
				Map<Integer, Integer> dstToPriceMap = srcToDstPriceMap.getOrDefault(currCity, new HashMap<>());
