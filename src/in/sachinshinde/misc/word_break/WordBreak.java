...
public class WordBreak {
  ...
  HashMap<String, Boolean> map =
      new HashMap<>(); // we must not use "boolean" here, should be "Boolean"
  ...
  private TrieNode buildTrie(List<String> wordDict) {
    ...
    for (String word : wordDict) {
      ...
      for (char ch : word.toCharArray()) {
        if (currNode.children[ch - 'a'] == null) currNode.children[ch - 'a'] = new TrieNode();
  ...
  public boolean wordBreak2(String s, List<String> wordDict) {
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      if (wordDict.contains(prefix) && wordBreak2(suffix, wordDict)) {
        map.put(s, true);
        return true;
      }
    }
