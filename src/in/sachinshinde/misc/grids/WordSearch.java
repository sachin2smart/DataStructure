...
public class WordSearch {
  ...
  public boolean exist(char[][] board, String word) {
    ...
    for (int i = 0; i < board.length; i++)
      for (int j = 0; j < board[i].length; j++)
        if ((word.charAt(0) == board[i][j]) && isWordFound(board, i, j, word, 0, visited))
          return true;
  ...
  private boolean isWordFound(
      char[][] board, int i, int j, String word, int wordIndex, boolean[][] visited) {
    ...
    if (isWordFound(board, i - 1, j, word, wordIndex + 1, visited)
        ...
        || isWordFound(board, i, j + 1, word, wordIndex + 1, visited)) return true;
  ...
  public boolean exist2(char[][] board, String word) {
    for (int x = 0; x < board.length; x++)
      for (int y = 0; y < board[x].length; y++)
        if (dfs(board, x, y, word.toCharArray(), 0)) return true;
