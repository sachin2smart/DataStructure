...
public class MinPathSum {
  ...
  private int getMinPathSum(int[][] grid, int m, int n) {
    ...
    if (m == 0) return grid[m][n] + getMinPathSum(grid, m, n - 1);

    if (n == 0) return grid[m][n] + getMinPathSum(grid, m - 1, n);

    return grid[m][n] + Math.min(getMinPathSum(grid, m, n - 1), getMinPathSum(grid, m - 1, n));
  }
  ...
  public int getMinPathSum2(int[][] grid) {
    ...
    for (int m = 0; m < height; m++) {
      for (int n = 0; n < width; n++) {
        ...
        else if (m == 0 && n != 0) grid[m][n] += grid[m][n - 1];
        else if (n == 0 && m != 0) grid[m][n] += grid[m - 1][n];
