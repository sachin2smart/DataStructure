...
public class NextGreaterElement2 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        Arrays.toString(nextGreaterElement2.nextGreaterElements(nums))); // 	2,3,-1,3,2
    System.out.println(
        Arrays.toString(nextGreaterElement2.nextGreaterElements2(nums))); // 	2,3,-1,3,2
    System.out.println(
        Arrays.toString(nextGreaterElement2.nextGreaterElements3(nums))); // 	2,3,-1,3,2
  }
  ...
  public int[] nextGreaterElements(int[] nums) {
    ...
    for (int i = 1; i <= nums.length; i++) {
      ...
      if (i == nums.length && !stack.isEmpty() && !isReset) {
        isReset =
            true; //	note: we don't want to iterate more than twice even if stack is not empty
        i = 0;
      }
      ...
      if (i < nums.length && !isReset) stack.push(i);
    }
  ...
  public int[] nextGreaterElements3(int[] nums) {
    ...
    Deque<Integer> dq =
        new LinkedList<>(); // you can use Stack here; but is not good for performance
