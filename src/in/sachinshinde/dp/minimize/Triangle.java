...
public class Triangle {
  public static void main(String[] args) {
    ...
    System.out.println(triangle.minimumTotal(Arrays.asList(Arrays.asList(-10)))); // 	-10
  ...
  private int recurse(List<List<Integer>> triangle, int row, int col, int[][] memo) {
    ...
    // either go to [row + 1, col] or [row + 1, col + 1]
    int remain =
        Math.min(recurse(triangle, row + 1, col, memo), recurse(triangle, row + 1, col + 1, memo));
  ...
  public int minimumTotal4(List<List<Integer>> triangle) {
    ...
    for (List<Integer> row : triangle) table.add(new ArrayList<>(row));

    for (int row = triangle.size() - 2; row >= 0; row--) {
      for (int col = 0; col < triangle.get(row).size(); col++) {
        int remain = Math.min(table.get(row + 1).get(col), table.get(row + 1).get(col + 1));
