...
/* Given an integer array nums, return the length of the longest strictly increasing subsequence.
	...
	For example, [0,1,2,7] is a subsequence of the array [0,3,1,6,2,2,7].
*/
...
public class LongestIncreasingSubsequence {
  ...
  public static void main(String[] args) {
    System.out.println(getLengthOfLIS(new int[] {10, 9, 2, 5, 3, 7, 101, 18})); // 4 <==[2,3,7,18]
    System.out.println(getLengthOfLIS(new int[] {0, 1, 0, 3, 2, 3})); // 4 <==[0,1,2,3]
    System.out.println(getLengthOfLIS(new int[] {7, 7, 7, 7, 7, 7})); // 1 <==[7]
  ...
  private static int getLength(int[] nums, int i, int prev) {
    ...
    if (nums[i] > prev) take = 1 + getLength(nums, i + 1, nums[i]);
  ...
  private static int getLength_DP_Mem(int[] nums, int i, int prev) {
    ...
    if (prev == -1 || nums[i] > nums[prev]) take = 1 + getLength_DP_Mem(nums, i + 1, i);
  ...
  private static int getLength_DP_Mem_SO(int[] nums, int i, int prev) {
    ...
    if (dpSO[prev + 1] != -1) return dpSO[prev + 1];

    int dontTake = getLength_DP_Mem_SO(nums, i + 1, prev);
    int take = 0;

    if (prev == -1 || nums[i] > nums[prev]) take = 1 + getLength_DP_Mem_SO(nums, i + 1, i);
  ...
  public static int getLengthOfLIS_tabulation(int[] nums) {
    ...
    // Bottom-up approach
    for (int i = 0; i < n; i++)
      for (int j = 0; j < i; j++) if (nums[i] > nums[j] && lis[i] < lis[j] + 1) lis[i] = lis[j] + 1;
      ...
      if (nums[i] > tail[currTarget - 1]) {
        ...
        int idx =
            Arrays.binarySearch(
                ...
                0,
                currTarget - 1,
                nums[i]); // Find largest value just smalller than nums[i] in tail
        if (idx < 0)
          idx =
              -1 * idx
                  - 1; // idx can be -ve, if element is not found by binary search, hence update
                       // index
        tail[idx] = nums[i];
      }
    }
  ...
  public static int getLengthOfLIS_nlogn_optimal(int[] nums) {
    ...
    for (int x : nums) {
      int i = Arrays.binarySearch(f, 0, n, x);
