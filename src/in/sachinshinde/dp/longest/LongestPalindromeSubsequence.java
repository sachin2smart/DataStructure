...
public class LongestPalindromeSubsequence {
  ...
  public int calculateRecursive(String str, int start, int len) {
    ...
    if (str.charAt(start) == str.charAt(start + len - 1))
      ...
      return Math.max(
          calculateRecursive(str, start + 1, len - 1), calculateRecursive(str, start, len - 1));
  }
  ...
  private int getSubLength(String s, int i, int j, Integer[][] dp) {
    ...
    if (s.charAt(i) == s.charAt(j)) dp[i][j] = getSubLength(s, i + 1, j - 1, dp) + 2;
    else dp[i][j] = Math.max(getSubLength(s, i + 1, j, dp), getSubLength(s, i, j - 1, dp));
