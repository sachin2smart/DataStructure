...
public class LongestCommonSubsequence {
    ...
    for (int i = 1; i <= m; i++) {
      for (int j = 1; j <= n; j++) {
        if (s[i - 1]
            == t[
                j
                    - 1]) { // if character matches means we found the same character in a sequence
                            // while interacting over both of char arrays
          dp[i][j] =
              1
                  + dp[i - 1][
                      j
                          - 1]; // one character matches hence +1 with whatever result we hold in
                                // dp[i-1][j-1]
        } else {
  ...
  private int getSubsequenceLength(String text1, String text2) {
    ...
    for (int i = 0; i < text1.length(); i++)
      for (int j = 0; j < text2.length(); j++)
        m[i + 1][j + 1] =
            text1.charAt(i) == text2.charAt(j) ? m[i][j] + 1 : Math.max(m[i + 1][j], m[i][j + 1]);
  ...
  public int longestCommonSubsequence(String text1, String text2) {
    ...
    for (int j = n - 1; j >= 0; j--) {
      for (int i = m - 1; i >= 0; i--) {
        if (t1[i] == t2[j]) curi[i] = 1 + prev[i + 1];
        else curi[i] = Math.max(prev[i], curi[i + 1]);
      }
      int[] temp = prev;
      prev = curi;
      curi = temp;
