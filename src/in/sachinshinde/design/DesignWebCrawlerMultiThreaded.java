...
public class DesignWebCrawlerMultiThreaded {
  public static void main(String[] args) {
    DesignWebCrawlerMultiThreaded designWebCrawlerMultithreaded =
        new DesignWebCrawlerMultiThreaded();

    List<String> yahooLinks =
        Arrays.asList(
            "http://news.yahoo.com",
            "http://news.yahoo.com/news",
            "http://news.yahoo.com/news/topics/",
    ...
    System.out.println(
        designWebCrawlerMultithreaded.crawl(
            "http://news.yahoo.com/news/topics/", websiteHtmlParser));
    //  [http://news.yahoo.com/news/topics/, http://news.yahoo.com/news, http://news.yahoo.com,
    // http://news.yahoo.com/us]

    System.out.println(
        designWebCrawlerMultithreaded.crawl("http://news.google.com", websiteHtmlParser));
    //  [http://news.google.com, http://news.google.com/search/]

    List<String> wlybyLinks =
        Arrays.asList("http://psn.wlyby.edu/upkr", "http://psn.wlyby.edu/ubmr");
    websiteHtmlParser = new WebsiteHtmlParser(wlybyLinks);
    System.out.println(
        designWebCrawlerMultithreaded.crawl("http://psn.wlyby.edu/ubmr", websiteHtmlParser));
  ...
  public List<String> crawl(String startUrl, HtmlParser htmlParser) {
    ...
    return crawl(startUrl, htmlParser, visited, host).collect(Collectors.toList());
  }

  private Stream<String> crawl(
      String startUrl, HtmlParser parser, Set<String> visited, String host) {
    Stream<String> stream =
        parser.getUrls(startUrl).parallelStream() //  parallelStream is multithreading
