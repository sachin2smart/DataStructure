...
/*
        ...
        It is guaranteed that there is always possible to find
            a binary search tree with the given requirements for the given test cases.
...
public class ConstructBSTFromInorderTraversal {
    ...
    public Node bstFromPreorder(int[] preorder, int maxVal, int[] currIndex) {
        if (currIndex[0] == preorder.length || preorder[currIndex[0]] > maxVal) {
            return null;
        }

        Node root = new Node(preorder[currIndex[0]++]);
        root.left = bstFromPreorder(preorder, root.key, currIndex);
        root.right = bstFromPreorder(preorder, maxVal, currIndex);
