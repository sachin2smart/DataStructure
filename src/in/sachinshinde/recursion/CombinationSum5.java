...
public class CombinationSum5 {
  ...
  public static void main(String[] args) {
    ...
    //	[[1, 2, 6], [1, 3, 5], [1, 4, 4], [1, 5, 3], [2, 2, 5], [2, 3, 4], [2, 4, 3], [3, 2, 4], [3,
    // 3, 3], [4, 2, 3]]
  ...
  private void constructSubsets(
      int k, int n, int start, List<Integer> currSet, List<List<Integer>> subsets) {
    if (k == currSet.size()) {
      if (n == 0) {
    ...
    for (int i = start; i <= 9; i++) {
      currSet.add(i);
      constructSubsets(
          k, n - i, start + 1, currSet, subsets); // start from start+1 for next recursion
