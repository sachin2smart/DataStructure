...
public class CombinationSum2 {
  ...
  private static void constructSubsets(
      int start, int[] nums, List<Integer> tempList, List<List<Integer>> subsets, int remain) {
    if (remain < 0) return;
    else if (remain == 0) {
  ...
  public static void main(String args[]) {
    ...
    for (List<Integer> tupple : ans) System.out.println(" " + tupple);
  ...
  private void constructSubsets(
      int start, int[] nums, int target, List<List<Integer>> subsets, List<Integer> currSumArray) {
    if (target == 0) {
