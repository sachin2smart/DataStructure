...
public class SurroundedRegions {
  ...
  private void firstLastColumnFlipper(char[][] board, int m, int n) {
    for (int i = 0; i < m; i++) {
      if (board[i][0] == 'O') starFlipper(board, i, 0);

      if (board[i][n - 1] == 'O') starFlipper(board, i, n - 1);
    }
  }

  private void firstLastRowFlipper(char[][] board, int m, int n) {
    for (int j = 1; j < n - 1; j++) {
      if (board[0][j] == 'O') starFlipper(board, 0, j);

      if (board[m - 1][j] == 'O') starFlipper(board, m - 1, j);
    }
  }
