...
public class LargestRectangleInHistogram {
  ...
  public int largestRectangleArea(int[] heights) {
    ...
    for (int i = 1; i < n; i++) {
      ...
      while (p >= 0 && heights[p] >= heights[i]) p = lessValueIndexFromLeft[p];
    ...
    for (int i = n - 2; i >= 0; i--) {
      ...
      while (p < n && heights[p] >= heights[i]) p = lessValueIndexFromRight[p];
    ...
    for (int i = 0; i < n; i++)
      maxArea =
          Math.max(
              maxArea,
              heights[i] * (lessValueIndexFromRight[i] - lessValueIndexFromLeft[i] - 1)); // width
