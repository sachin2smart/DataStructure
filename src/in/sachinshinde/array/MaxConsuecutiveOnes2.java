...
public class MaxConsuecutiveOnes2 {
  public static void main(String[] args) {
    MaxConsuecutiveOnes2 maxConsuecutiveOnes = new MaxConsuecutiveOnes2();

    System.out.println(maxConsuecutiveOnes.findMaxConsecutiveOnes(new int[] {1, 0, 1, 1, 0})); // 	4
    System.out.println(
        maxConsuecutiveOnes.findMaxConsecutiveOnes_forLargeData(new int[] {1, 0, 1, 1, 0})); // 	4
    System.out.println(
        maxConsuecutiveOnes.findMaxConsecutiveOnes2(new int[] {1, 0, 1, 1, 0})); // 	4
  }
  ...
  */
  ...
  public int findMaxConsecutiveOnes(int[] nums) {
    ...
    for (int right = 0; right < nums.length; ++right) {
      ...
      while (numsZeros > k) if (nums[left++] == 0) --numsZeros;
  ...
  public int findMaxConsecutiveOnes2(int[] nums) {
    ...
    while (index < totalNums) {
      if (nums[index] == 1) {
        ...
        while (index < totalNums && nums[index++] == 1) currentWindowSize++;

        int sumHere = currentWindowSize + previousWindowSize;

        if (currentWindowSize != index || index < totalNums) ++sumHere;
