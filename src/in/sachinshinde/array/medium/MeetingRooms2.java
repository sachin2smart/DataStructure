...
public class MeetingRooms2 {
  ...
  private int minMeetingRooms1(Interval[] intervals) {
    ...
    if (intervals[i - 1].end > intervals[i].start) count++;
  ...
  private int minMeetingRooms2(Interval[] intervals) {
    ...
    // the mapping value is decreased first and then increased and remains at 0, no new room is
    // allocated
  ...
  private int minMeetingRooms3(Interval[] intervals) {
    ...
    for (int i = 1; i < intervals.length; i++) { // start from second sorted interval i=1
      if (intervals[i].start < pq.peek()) {
        ...
        pq.offer(
            Math.max(
                intervals[i].end, pq.poll())); // poll then offer, conceptually merging 2 intervals
      }
    }
