...
public class DailyTempratures {
  public static void main(String[] args) {
    ...
    System.out.println(
        Arrays.toString(dt.dailyTemperatures(new int[] {73, 74, 75, 71, 69, 72, 76, 73})));
    System.out.println(
        Arrays.toString(dt.dailyTemperatures2(new int[] {73, 74, 75, 71, 69, 72, 76, 73})));
    System.out.println(
        Arrays.toString(dt.dailyTemperatures3(new int[] {73, 74, 75, 71, 69, 72, 76, 73})));

    System.out.println(
        Arrays.toString(dt.dailyTemperatures(new int[] {30, 40, 50, 60}))); // [1, 1, 1, 0]
    System.out.println(
        Arrays.toString(dt.dailyTemperatures2(new int[] {30, 40, 50, 60}))); // [1, 1, 1, 0]
    System.out.println(
        Arrays.toString(dt.dailyTemperatures3(new int[] {30, 40, 50, 60}))); // [1, 1, 1, 0]
  }

  //	using stack
  public int[] dailyTemperatures(int[] temperatures) {
    Stack<Integer> st = new Stack<>(); // stack of index
    int[] ans = new int[temperatures.length]; // initially all 0

    for (int currDay = 0; currDay < temperatures.length; currDay++) {
      while (!st.isEmpty() && temperatures[currDay] > temperatures[st.peek()]) {
        int lastHighTempDay = st.pop();
        ans[lastHighTempDay] = currDay - lastHighTempDay;
      }
      st.push(currDay);
    }
    return ans;
