...
public class HouseRobber {
  ...
  private int rob1Recursive(int[] nums, int i) {
    if (i < 0) return 0;
    return Math.max(rob1Recursive(nums, i - 2) + nums[i], rob1Recursive(nums, i - 1));
  }
  ...
  private int rob2Recursive(int[] nums, int i) {
    ...
    int result = Math.max(rob2Recursive(nums, i - 2) + nums[i], rob2Recursive(nums, i - 1));
  ...
  public int rob3(int[] nums) {
    ...
    for (int i = 1; i < nums.length; i++) memo[i + 1] = Math.max(memo[i], memo[i - 1] + nums[i]);
