...
public class KokoEatingBanana {
  ...
  public int minEatingSpeed(int[] piles, int h) {
    ...
    while (left < right) {
      ...
      int totalPiles = 0;
      for (int pile : piles) totalPiles += (mid + pile - 1) / mid;
      if (totalPiles > h) left = mid + 1;
      else right = mid;
    }
  ...
  public int minEatingSpeed2(int[] piles, int h) {
    /*
           // Better Performance wth deciding left and right pointers
           ...
           int right = (int) ((sumPiles - n) / (h - n + 1)) + 1;
    ...
    */
    ...
    return left; // answer exists in left pointer
  }
}
