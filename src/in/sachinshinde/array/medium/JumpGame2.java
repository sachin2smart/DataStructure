...
public class JumpGame2 {
  ...
  public int jump(int[] nums) {
    ...
    int currCoveragePos =
        0; // why this extra variable ? --> To indicate the max jump from the current position
    int lastJumpPos =
        0; // purpose of this variable ? --> To indicate till what position we were reached from
           // last jump
  ...
  public int jump2(int[] nums) {
    ...
    for (int i = 1; i < n; i++) {
      nums[i] =
          Math.max(nums[i] + i, nums[i - 1]); // if the modification to the input array is allowed
    }
