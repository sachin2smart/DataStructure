...
public class MaximumScorePossibleAfterPerformingGivenOperationsOnAnArray {
  ...
  */
  ...
  //  dp[l][r][num] = where l and r represent the endpoints of the current array and num represents
  // the operation number.
  private final int[][][] dp = new int[100][100][100];

  public static void main(String[] args) {
    MaximumScorePossibleAfterPerformingGivenOperationsOnAnArray maximumScore =
        new MaximumScorePossibleAfterPerformingGivenOperationsOnAnArray();
    System.out.println(maximumScore.findMaxScore(new int[] {1, 2, 3, 4, 2, 6})); //  13
    System.out.println(
        maximumScore.findMaxScore(new int[] {5, 2, 2, 8, 1, 16, 7, 9, 12, 4})); //  50

    System.out.println(maximumScore.findMaxScore2(new int[] {1, 2, 3, 4, 2, 6})); //  13
    System.out.println(
        maximumScore.findMaxScore2(new int[] {5, 2, 2, 8, 1, 16, 7, 9, 12, 4})); //  50

    System.out.println(maximumScore.findMaxScore3(new int[] {1, 2, 3, 4, 2, 6})); //  13
    System.out.println(
        maximumScore.findMaxScore3(new int[] {5, 2, 2, 8, 1, 16, 7, 9, 12, 4})); //  50

    System.out.println(maximumScore.findMaxScore4(new int[] {1, 2, 3, 4, 2, 6})); //  13
    System.out.println(
        maximumScore.findMaxScore4(new int[] {5, 2, 2, 8, 1, 16, 7, 9, 12, 4})); //  50
  }
  ...
  private int maxScore(int l, int r, int[] prefixSum, int num) {
    ...
    // Exploring all paths, by removing leftmost and rightmost element and selecting the maximum
    // value
    return currentSum
        + Math.max(maxScore(l + 1, r, prefixSum, num + 1), maxScore(l, r - 1, prefixSum, num + 1));
  }
  ...
  */
  ...
  private int MaximumScoreDP(int l, int r, int[] prefixSum, int num) {
    ...
    // Exploring all paths, and storing maximum value in DP table to avoid further repetitive
    // recursive calls
    dp[l][r][num] =
        currentSum
            + Math.max(
                MaximumScoreDP(l + 1, r, prefixSum, num + 1),
      ...
      for (int i = 0; i + l - 1 < n; i++) {
        ...
        for (int num = 1; num <= n; num++) {
          ...
          if (i == j) {
            ...
            dp[i][j][num] =
                Math.max(
                    currentSum + dp[i + 1][j][num + 1],
                    currentSum
                        + dp[i][j - 1][
                            num
                                + 1]); //  // Exploring all paths, and storing maximum value in DP
                                       // table
          }
        }
      }
