...
public class TimeZoneApiCalls {
  ...
  public static void main(String[] args) {
    long offsetLondon =
        TimeZone.getTimeZone("Europe/London").getOffset(Instant.now().toEpochMilli());
    System.out.println("London Offset : " + offsetLondon);

    long offsetNewyork =
        TimeZone.getTimeZone("America/New_York").getOffset(Instant.now().toEpochMilli())
            / 1000
            / 60;
    System.out.println("New_York Offset : " + offsetNewyork);

    long offsetMontreal =
        TimeZone.getTimeZone("America/Montreal").getOffset(new Date().getTime()) / 1000 / 60;
    ...
    System.out.println(
        "GMT Offset Minutes: "
            + Duration.ofSeconds(
                    ZoneOffset.from(GMT_OFFSET_PARSER.parse("GMT-05:00")).getTotalSeconds())
                .toMinutes());
  }
}
