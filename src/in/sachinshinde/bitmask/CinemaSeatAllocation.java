...
public class CinemaSeatAllocation {
  ...
  public static void main(String[] args) {
    ...
    reservedSeats = new int[][] {{1, 8}, {2, 1}, {2, 6}};
  ...
  public int maxNumberOfFamilies(int n, int[][] reservedSeats) {
    ...
    for (int[] reserved : reservedSeats)
      reservationMap.put(
          reserved[0], reservationMap.getOrDefault(reserved[0], 0) | (1 << reserved[1]));
    ...
    for (int row : reservationMap.keySet()) {
      ...
      if ((reserved & 60) == 0) numTicketsFromCurrentRow += 1;

      // check if seats 6,7,8,9 are available
      if ((reserved & 960) == 0) numTicketsFromCurrentRow += 1;

      // check if seats 4,5,6,7 are available
      if ((reserved & 240) == 0 && numTicketsFromCurrentRow == 0) numTicketsFromCurrentRow = 1;
  ...
  public int maxNumberOfFamilies2(int n, int[][] reservedSeats) {
    ...
    for (int[] slots : seats.values()) {
      ...
      if (taken == 2) { // both slots at either ends are taken
        ...
        else availableSlots -= 2; // entire row not available - reduce by 2
      ...
      else continue; // entire row is available - no need to reduce the available slots
    }
