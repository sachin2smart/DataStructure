...
public class LowestCommonAncestorOfBinaryTreeForDeepestLeaves {
  ...
  private int getDepth(Node node, int depth) {
    ...
    if (leftTreeDepth == deepest && rightTreeDepth == deepest) {
      lca = node; // if the left and right child are deepest node of a tree then their parent node
      // will be
    }
  ...
  public int heightMemorized(Node root) {
    ...
    heights.put(root, 1 + Math.max(heightMemorized(root.left), heightMemorized(root.right)));

    return heights.get(root);
  }

  public Node lcaDeepestLeaves3(Node root) {
    if (root == null || heightMemorized(root.right) == heightMemorized(root.left)) return root;

    return lcaDeepestLeaves3(
        heightMemorized(root.left) > heightMemorized(root.right) ? root.left : root.right);
  }
