...
public class PathSum3 {
  ...
  public int pathSumHelper2(Node root, int sum) {
    ...
    int count = 0;
    if (root.key == sum) count++;

    return count
        + pathSumHelper2(root.left, sum - root.key)
        + pathSumHelper2(root.right, sum - root.key);
  }
  ...
  private int getCount(
      Node node, HashMap<Long, Integer> sum, long currSum, int count, int targetSum) {
    currSum = node.key + currSum;
    long target = currSum - targetSum;

    if (target == 0) count++;

    if (sum.containsKey(target)) count += sum.get(target);

    if (node.left == null && node.right == null) return count;

    if (sum.containsKey(currSum)) sum.put(currSum, sum.get(currSum) + 1);
    else sum.put(currSum, 1);

    if (node.left != null) count = getCount(node.left, sum, currSum, count, targetSum);

    if (node.right != null) count = getCount(node.right, sum, currSum, count, targetSum);

    // remove itself so that it won't be involved for further calculation
    int actualSum = sum.get(currSum) - 1;
    if (actualSum > 0) sum.put(currSum, actualSum);
    else sum.remove(currSum);
  ...
  public void preOrder(
      Node root, int currSum, int targetSum, HashMap<Integer, Integer> prefixSumMap) {
    if (root == null) return;

    currSum += root.key;

    if (currSum == targetSum) numPaths++;
