...
public class DistanceK {
    ...
    while (!q.isEmpty()) {
      if (currLevel != k) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (parentTracker.get(currNode) != null
              && visited.get(parentTracker.get(currNode)) == null) {
  ...
  private void accumulateKDistanceNodes(
      Node target,
    ...
    if (parentTracker.containsKey(target)
        && parentTracker.get(target) != null
