...
public class SumTree {
  ...
  static boolean isSumTree(Node node) {
    ...
    if (node == null || isLeafNode(node)) return true;

    int ls, rs;

    if (isSumTree(node.left) && isSumTree(node.right)) {
      if (node.left == null) ls = 0;
      else if (isLeafNode(node.left)) ls = node.left.key;
      else ls = 2 * node.left.key;

      if (node.right == null) rs = 0;
      else if (isLeafNode(node.right)) rs = node.right.key;
      else rs = 2 * node.right.key;

      if (node.key == ls + rs) return true;
      else return false;
    }
