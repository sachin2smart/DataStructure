...
public class CheckIfAStringIsAValidSequenceFromRootToLeavesInBinaryTree {
  ...
  public static void main(String[] args) {
    CheckIfAStringIsAValidSequenceFromRootToLeavesInBinaryTree check =
        new CheckIfAStringIsAValidSequenceFromRootToLeavesInBinaryTree();
  ...
  private boolean dfs(Node root, int[] arr, int currLen) {
    if (currLen >= arr.length) return false;

    if (root == null || root.key != arr[currLen]) return false;

    if (currLen == arr.length - 1) return root.left == null && root.right == null;

    return dfs(root.left, arr, currLen + 1) || dfs(root.right, arr, currLen + 1);
  }

  // 2. Using BFS
  private boolean isValidSequence2(Node root, int[] arr) {
    if (root == null || root.key != arr[0]) return false;
    ...
    while (!queue.isEmpty() & currIndex < arr.length) {
      ...
      for (int i = 0; i < qSize; i++) {
        ...
        if (n.key == arr[currIndex]) {
          ...
          if (n.left == null && n.right == null && currIndex == arr.length - 1) return true;
        }
      }
      currIndex++;
    }
