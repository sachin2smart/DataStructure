...
According to what we have above, we should first arrange A, and then B and C.
...
Now if we have emptySlots > availableTasks
...
Almost done.
One special case: what if there are more than one task with most frequency?
...
What if we have more than n tasks with most frequency and we got emptySlot negative?
	...
	Like 3A, 3B, 3C, 3D, 3E, n = 2.

In this case seems like we can't put all B C S inside slots since we only have n = 2.
Well partCount is actually the "minimum" length of each part required for arranging A.
You can always make the length of part longer.
...
in this case you have already met the "minimum" length requirement for each part (n = 2),
you can always put more tasks in each part if you like:
...
public class TaskScheduler {
  public static void main(String[] args) {
    ...
    System.out.println(scheduler.leastInterval(new char[] {'A', 'A', 'A', 'B', 'B', 'B'}, 2)); // 	8
    System.out.println(scheduler.leastInterval(new char[] {'A', 'A', 'A', 'B', 'B', 'B'}, 0)); // 	6
    System.out.println(
        scheduler.leastInterval(
            new char[] {'A', 'A', 'A', 'A', 'A', 'A', 'B', 'C', 'D', 'E', 'F', 'G'}, 2)); // 16
    System.out.println(
        scheduler.leastInterval(
            new char[] {'A', 'A', 'A', 'B', 'B', 'C', 'C', 'D', 'D', 'F', 'F'}, 2)); // 11

    System.out.println(
        scheduler.leastInterval2(new char[] {'A', 'A', 'A', 'B', 'B', 'B'}, 2)); // 	8
    System.out.println(
        scheduler.leastInterval2(new char[] {'A', 'A', 'A', 'B', 'B', 'B'}, 0)); // 	6
    System.out.println(
        scheduler.leastInterval2(
            new char[] {'A', 'A', 'A', 'A', 'A', 'A', 'B', 'C', 'D', 'E', 'F', 'G'}, 2)); // 16
    System.out.println(
        scheduler.leastInterval2(
            new char[] {'A', 'A', 'A', 'B', 'B', 'C', 'C', 'D', 'D', 'F', 'F'}, 2)); // 11
  ...
  public int leastInterval2(char[] tasks, int n) {
    ...
    int maxFreq = freq[25];
    int idelTime = (maxFreq - 1) * n;

    for (int i = freq.length - 2; i >= 0 && idelTime > 0; i--)
      idelTime -= Math.min(maxFreq - 1, freq[i]);

    idelTime = Math.max(idelTime, 0);
