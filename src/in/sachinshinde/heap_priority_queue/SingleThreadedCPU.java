...
public class SingleThreadedCPU {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        Arrays.toString(
            singleThreadedCPU.getOrder(
                new int[][] {{1, 2}, {2, 4}, {3, 2}, {4, 1}}))); //  [0, 2, 3, 1]
    System.out.println(
        Arrays.toString(
            singleThreadedCPU.getOrder(
                new int[][] {{7, 10}, {7, 12}, {7, 5}, {7, 4}, {7, 2}}))); //  [4, 3, 2, 0, 1]
  }
  ...
  public int[] getOrder(int[][] tasks) {
    ...
    Arrays.sort(
        orders,
        (a, b) -> a[1] - b[1]); // the ones start earlier should be added into the queue earlier
    PriorityQueue<int[]> queue =
        new PriorityQueue<>(
            (a, b) ->
                a[2] == b[2]
                    ? a[0] - b[0]
                    : a[2] - b[2]); // sort firstly by processing time, then index

    int j = 0, time = 0, i = 0;
    int[] res = new int[n];

    while (j
        < n) { // note better use j than i, because all orders could be added into queue, then i==n
      // but j's not. Or here we can do i < n && !queue.isEmpty()
      if (queue.isEmpty()) {
        time =
            Math.max(
                time, orders[i][1]); // the previous task done time or next task's starting time
      }
