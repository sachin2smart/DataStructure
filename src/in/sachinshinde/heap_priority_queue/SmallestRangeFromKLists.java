...
public class SmallestRangeFromKLists {
        ...
        Arrays.toString(
            range.smallestRange(
                Arrays.asList(
                    ...
                    Arrays.asList(5, 18, 22, 30))))); // ans :  [20, 24]
  ...
  public int[] smallestRange(List<List<Integer>> nums) {
    ...
    // Find the max value from all lit, i.e. nums.get(i).get(0) where i belongs to [0,
    // nums.size()-1]
    ...
    while (true) {
      ...
      for (int i = 0; i < nums.size(); i++) {
        while (next[i] + 1 < nums.get(i).size() && nums.get(i).get(next[i] + 1) <= max) {
          next[i]++;
        }
